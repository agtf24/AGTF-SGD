...
def ssgd_with_horovod(dnn, dataset, data_dir, nworkers, lr, batch_size, nsteps_update, max_epochs, nwpernode, pretrain, num_steps, compressor, density, threshold):
    ...
    if settings.ADAPTIVE_MERGE or settings.ADAPTIVE_SPARSE: # 需要在设置中修改两个变量为True
        seq_layernames, layerwise_times, layerwise_sizes, layerwise_norm = benchmark(trainer) # 增加获得每层的梯度二范数
        ...
        if rank == 0:
            ...
            logger.info('layerwise backward grad-norms: %s', list(layerwise_norm))
        logger.info('Bencharmked backward time: %f', np.sum(layerwise_times))
        logger.info('Model size: %d', np.sum(layerwise_sizes))
    else:
        seq_layernames, layerwise_times, layerwise_sizes, layerwise_norm= None, None, None, None
    ...
    optimizer = hvd.DistributedOptimizer(trainer.optimizer, named_parameters=trainer.net.named_parameters(), compression=compressors[compressor], is_sparse=is_sparse, density=density, seq_layernames=seq_layernames, layerwise_times=layerwise_times, layerwise_norm =layerwise_norm, norm_clip=norm_clip, threshold=threshold, writer=writer)
