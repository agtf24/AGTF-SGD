...
class Profiling(object):
    ...
    def _make_hook(self, name, p):
        def hook(*ignore):
            ...
            if name not in self._handles:
                self._handles[name] = []
            if name not in self._grad_norm:
                self._grad_norm[name] = []
            torch.cuda.synchronize()
            p_grad_cpu = p.grad.data.cpu() # 现将梯度传到cpu中
            self._grad_norm[name].append(np.linalg.norm(p_grad_cpu)) # 获得梯度的二范数
    ...
    def get_layerwise_grad_norm(self): # 获得50个iteration梯度的平均值，将这个值作为梯度权重的代表
        num_trials = len(self._handles[self._seq_keys[0]])
        layerwise_grad_norm_multipletest = []
        for j in range(num_trials):
            g = 0
            layerwise_grad_norm = []
            for i, k in enumerate(self._backward_seq_keys):
                g = self._grad_norm[k][j]
                layerwise_grad_norm.append(g)
            layerwise_grad_norm_multipletest.append(layerwise_grad_norm)
        array = np.array(layerwise_grad_norm_multipletest)
        layerwise_grad_norm = np.mean(array, axis=0)
        return layerwise_grad_norm
...
def benchmark(trainer):
    ...
    grad_norm = p.get_layerwise_grad_norm() # 获取梯度范数
    p.stop()
    #return np.array([t for n,t in layerwise_times])
    return seq_keys[::-1], layerwise_times[::-1], p.get_backward_key_sizes()[::-1], grad_norm[::-1]
